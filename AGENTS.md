...
| [`src/server/json.ts`](src/server/json.ts) | `MCP_JSON_FORMAT_VERSION="4"`, `jsonRespond()` (minified), `errorRespond`, `mkError`, `mkLocalRepoNoRemote`, `spreadDefined`, `truncateLines`, `truncateText` |
...
- **`MCP_JSON_FORMAT_VERSION`** (now `"4"`): bump on incompatible JSON changes.
