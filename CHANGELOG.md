...
- **`workflow_dispatch`**: optional `watch` (default `false`) and `timeoutSec` (5–300, default `60`) parameters. With `watch: true`, the tool polls for the dispatched run and its completion before returning, adding `runId`, `url`, `status`, and `conclusion` to the response (`timedOut: true` if the deadline passes first). `watch: false` (the default) keeps the existing message-only response unchanged.
